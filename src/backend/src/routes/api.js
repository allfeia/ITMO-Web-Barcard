...
router.get('/me', authRequired, async (req, res) => {
  try {
    ...
    const sumRow = await Point.findOne({
      ...
      attributes: [[fn('COALESCE', fn('SUM', col('points')), 0), 'total_points']]
    });
...
router.get('/admin/bars', authRequired, requireRole('super_admin'), async (req, res) => {
  const bars = await Bar.findAll({ order: [['name', 'ASC']] });
  res.json(bars.map(b => ({ id: b.id, name: b.name })));
});

router.get('/bars', authRequired, requireRole('super_admin'), async (req, res) => {
...
router.post('/admin/bars', authRequired, requireRole('super_admin'), async (req, res) => {
  try {
    const { name, address, description, website, barKey } = adminBarSchema.parse(req.body);
...
router.get('/admin/bars/:barId/staff', authRequired, requireRole('super_admin'), async (req, res) => {
