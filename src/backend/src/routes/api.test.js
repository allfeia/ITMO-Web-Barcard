...
vi.mock('../models.js', () => {
  return {
    Bar: { findAll: vi.fn(), findByPk: vi.fn(), create: vi.fn(), findOne: vi.fn() },
...
describe('api router', () => {
  ...
  describe('GET /me', () => {
    ...
    it('returns profile with points sum', async () => {
      const fakeUser = { id: 2, email: 'a', login: 'l', name: 'n', roles: ['user'], bar_id: 3 };
  ...
  describe('GET /admin/bars', () => {
    it('requires super_admin', async () => {
      const app = appWithRouter();
      const auth = bearer({ id: 1, roles: ['user'] });
      const res = await request(app).get('/api/admin/bars').set('Authorization', auth);
      expect(res.status).toBe(403);
    });

    it('lists bars', async () => {
      Bar.findAll.mockResolvedValue([{ id: 1, name: 'A' }, { id: 2, name: 'B' }]);
      const app = appWithRouter();
      const auth = bearer({ id: 1, roles: ['super_admin'] });
      const res = await request(app).get('/api/admin/bars').set('Authorization', auth);
      expect(res.status).toBe(200);
      expect(res.body).toEqual([{ id: 1, name: 'A' }, { id: 2, name: 'B' }]);
  ...
  describe('GET /bars?query=', () => {
    it('returns [] for empty query', async () => {
      const app = appWithRouter();
      const auth = bearer({ id: 1, roles: ['super_admin'] });
      const res = await request(app).get('/api/bars').set('Authorization', auth);
      expect(res.status).toBe(200);
      expect(res.body).toEqual([]);
    });
    ...
    it('searches by name (ilike)', async () => {
      ...
      const app = appWithRouter();
      const auth = bearer({ id: 1, roles: ['super_admin'] });
      const res = await request(app).get('/api/bars?query=Fo').set('Authorization', auth);
      expect(res.status).toBe(200);
  ...
  describe('POST /admin/bars', () => {
    ...
    it('201 on valid payload', async () => {
      ...
      const res = await request(app)
        .post('/api/admin/bars')
        .set('Authorization', auth)
        .send({ name: 'NewBar', address: 'Street 1', description: 'desc', website: 'https://ex.com', barKey: 'k' });

      expect(res.status).toBe(201);
      expect(res.body).toEqual({ message: 'Бар создан', id: 10, name: 'NewBar' });
  ...
  describe('GET /admin/bars/:barId/staff', () => {
    ...
    it('returns mapped staff', async () => {
      ...
      User.findAll.mockResolvedValue([
        { id: 1, email: 'e1', login: 'l1', name: 'n1', roles: ['staff'], bar_id: 7 },
        { id: 2, email: 'e2', login: 'l2', name: 'n2', roles: ['bar_admin'], bar_id: 7 },
      ]);
      const app = appWithRouter();
      const auth = bearer({ id: 1, roles: ['super_admin'] });
      const res = await request(app)
