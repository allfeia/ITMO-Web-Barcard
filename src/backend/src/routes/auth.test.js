...
function bearer(user) {
  const token = jwt.sign(user, process.env.JWT_SECRET || 'dev', { expiresIn: '1h' });
  return `Bearer ${token}`;
}
...
describe('auth endpoints', () => {
  ...
  describe('POST /super/login', () => {
    ...
    it('404 when user not found', async () => {
      ...
      const app = appWithRouter();
      const res = await request(app).post('/auth/super/login').send({ username: 'root', password: 'x' });
    ...
    it('403 when not super_admin', async () => {
      ...
      const app = appWithRouter();
      const res = await request(app).post('/auth/super/login').send({ username: 'u', password: 'x' });
      expect(res.status).toBe(403);
      expect(res.body).toEqual({ error: 'Forbidden' });
    });

    it('403 when no password set', async () => {
      User.findOne.mockResolvedValue({ roles: ['super_admin'], password: null });
      const app = appWithRouter();
      const res = await request(app).post('/auth/super/login').send({ username: 'u', password: 'x' });
      expect(res.status).toBe(403);
    ...
    it('403 when invalid password', async () => {
      ...
      User.findOne.mockResolvedValue({ id: 1, login: 'l', email: 'e', name: 'n', roles: ['super_admin'], password: 'other' });
      const app = appWithRouter();
      const res = await request(app).post('/auth/super/login').send({ username: 'u', password: 'x' });
      expect(res.status).toBe(403);
    ...
    it('200 returns token & user', async () => {
      User.findOne.mockResolvedValue({
        id: 1, login: 'root', email: 'root@example.com', name: 'Root',
        roles: ['super_admin'], password: 'pass'
      });
      const app = appWithRouter();
      const res = await request(app).post('/auth/super/login').send({ username: 'root', password: 'pass' });
  ...
  describe('POST /barman/auth', () => {
    ...
    it('404 bar not found', async () => {
      ...
      const app = appWithRouter();
      const res = await request(app).post('/auth/barman/auth').send({ barId: 1, username: 'u', password: 'p', barKey: 'k' });
      expect(res.status).toBe(404);
    ...
    it('403 wrong bar key', async () => {
      ...
      const app = appWithRouter();
      const res = await request(app).post('/auth/barman/auth').send({ barId: 2, username: 'u', password: 'p', barKey: 'bad' });
      expect(res.status).toBe(403);
    ...
    it('404 user not found', async () => {
      ...
      const app = appWithRouter();
      const res = await request(app).post('/auth/barman/auth').send({ barId: 2, username: 'u', password: 'p', barKey: 'k' });
      expect(res.status).toBe(404);
    ...
    it('403 not staff', async () => {
      ...
      User.findOne.mockResolvedValue({ roles: ['user'], bar_id: 2, password: 'p' });
    ...
    it('403 attached to another bar', async () => {
      ...
      const { default: bcrypt } = await import('bcryptjs');
      bcrypt.compare.mockResolvedValueOnce(true); // ключ бара ок
      User.findOne.mockResolvedValue({ roles: ['staff'], bar_id: 3, password: 'p' });

      const app = appWithRouter();
      const res = await request(app).post('/auth/barman/auth').send({ barId: 2, username: 'u', password: 'p', barKey: 'k' });
      expect(res.status).toBe(403);
      expect(res.body).toEqual({ error: 'Пользователь привязан к другому бару' });
    });
    ...
    it('403 no password set', async () => {
      ...
      const { default: bcrypt } = await import('bcryptjs');
      bcrypt.compare.mockResolvedValueOnce(true); // ключ бара ок
      User.findOne.mockResolvedValue({ roles: ['staff'], bar_id: 2, password: null });
    ...
    it('403 wrong password', async () => {
      ...
      User.findOne.mockResolvedValue({ roles: ['staff'], bar_id: 2, password: 'other' });
    ...
    it('200 login ok', async () => {
      ...
      User.findOne.mockResolvedValue({
        id: 10, login: 'l', email: 'e', name: 'n',
        roles: ['staff'], bar_id: 2, password: 'p'
      });
  ...
  describe('POST /super/users/register-in-bar', () => {
    ...
    it('400 invalid payload (no barId/barName)', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          // нет barId/barName
          roles: ['staff'],
          name: 'N', login: 'l', email: 'e@example.com', password: 'secret12',
        });
      expect(res.status).toBe(400);
      expect(res.body).toEqual({ error: 'Invalid payload' });
    });
    ...
    it('404 if bar not found by id', async () => {
      ...
      const res = await request(app)
        ...
        .set('Authorization', bearer({ id: 1, roles: ['super_admin'] }))
        .send({
          barId: 5, roles: ['staff'], name: 'N', login: 'l', email: 'e@example.com', password: 'secret12',
        });
      expect(res.status).toBe(404);
      expect(res.body).toEqual({ error: 'Бар не найден' });
    });
    ...
    it('404 if bar not found by name', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          barName: 'X', roles: ['staff'], name: 'N', login: 'l', email: 'e@example.com', password: 'secret12',
        });
      expect(res.status).toBe(404);
      expect(res.body).toEqual({ error: 'Бар не найден' });
    });
    ...
    it('201 creates new user if not exists', async () => {
      ...
      U.User.create = vi.fn().mockResolvedValue({
        id: 10, email: 'e@example.com', login: 'l', name: 'N', roles: ['staff'], bar_id: 7,
      });
      ...
      const res = await request(app)
        ...
        .set('Authorization', bearer({ id: 1, roles: ['super_admin'] }))
        .send({
          barId: 7, roles: ['staff'], name: 'N', login: 'l', email: 'e@example.com', password: 'secret12',
        });
    ...
    it('201 updates existing user', async () => {
      ...
      const save = vi.fn();
      User.findOne.mockResolvedValue({
        id: 11, email: 'old@example.com', login: 'old', name: 'Old', roles: ['user'], password: 'hash:x', bar_id: null, save
      });
      ...
      const res = await request(app)
        ...
        .set('Authorization', bearer({ id: 1, roles: ['bar_admin'] }))
        .send({
          barId: 8, roles: ['staff', 'bar_admin'], name: 'New', login: 'new', email: 'new@example.com', password: 'secret12',
        });

      expect(res.status).toBe(201);
      expect(save).toHaveBeenCalled();
      expect(res.body.user).toMatchObject({ bar_id: 8, roles: ['staff', 'bar_admin'] });
    });
  });
});