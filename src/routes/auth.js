...
router.post('/super/login', async (req, res) => {
  try {
    ...
    if (!roles.has('super_admin')) return res.status(403).json({ error: 'Forbidden' });

    if (!user.password) return res.status(403).json({ error: 'No password set' });
    ...
    if (e.errors || e.issues) return res.status(400).json({ error: 'Invalid payload' });
...
router.post('/barman/auth', async (req, res) => {
  try {
    const { barId, username, password, barKey } = barmanAuthSchema.parse(req.body);
    ...
    const roles = new Set(user.roles || []);
    if (!roles.has('staff') && !roles.has('bar_admin')) return res.status(403).json({ error: 'Пользователь не сотрудник бара' });
    if (user.bar_id !== bar.id) return res.status(403).json({ error: 'Пользователь привязан к другому бару' });

    if (!user.password) return res.status(403).json({ error: 'Пароль не установлен' });
    const ok = await bcrypt.compare(password, user.password);
...
router.post('/super/grant-bar-admin', authRequired, requireRole('super_admin'), async (req, res) => {
  try {
    ...
    const roles = new Set(user.roles || []);
    if (makeBarAdmin) {
      if (!user.bar_id) return res.status(400).json({ error: 'Сотрудник должен работать в баре' });
      roles.add('bar_admin');
      roles.add('staff'); 
    } else {
    ...
    return res.json({ ok: true, user: { id: user.id, roles: user.roles, bar_id: user.bar_id } });
  } catch (e) {
    if (e.errors || e.issues) return res.status(400).json({ error: 'Invalid payload' });
    return res.status(500).json({ error: 'Server error' });
  }
...
const superRegisterInBarSchema = z.object({
  ...
  (v) => Boolean(v.barId) || Boolean(v.barName),
  { message: 'Нужно указать barId или barName', path: ['barId'] }
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      let user = await User.findOne({ where: { [Op.or]: [{ email }, { login }] } });
